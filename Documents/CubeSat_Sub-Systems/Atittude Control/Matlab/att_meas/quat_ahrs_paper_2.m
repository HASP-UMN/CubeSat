...
%   AHRS is in the form of periodic attitude measurements (e.g.
%   from a GPS short baseline attitude system).
%
%   This script primarily plots the Kalman gains.
%
%   Programmer:                     Demoz Gebre-Egziabher
%   Previously Modified:            February 22, 2003 
...
quat_error_vec = zeros(drl,4);

%-----------3.1   Define Filter Variables  ------%

loop_count = 0;
FILTER = 'ON';%'OFF';%
update_limit = 2;%round(0.5*1/dt);
...
Rv = eye(3)*(0.25*d2r/2)^2;                       % Measurement Noise Matrix
...
Gn2q = -0.5*eye(3);                             % "n" = wide band noise; "c" = colored noise.
Gc2q = zeros(3,3);
Gn2w = zeros(3,3);
Gc2w = eye(3);
...
for k = 2:drl
    ...
    %
    omega = [p_use q_use r_use]';
    %omega = [p q r]';
          
     % ===== Fe2e:  Quaternion to Quaternion Error Block ==== %
    ...
    F = [Fq2q    Fw2q;...
          Fq2w    Fw2w ];        % Dynamic Matirx at t = k
...
%   Form the M matrix (see research notes pp 24)
    ...
    M = [0 -omega';omega -sk(omega)];

%   Integrate the quaternion differential equation and update quaternions

    qhat = expm(0.5*dt*M)*qhat;          
    eul(k,:) = dcm2eul(quat2dcm(qhat))';

    quat_error_vec(k,:) = quat_error_vec(k-1,:); 
    ...
    if (loop_count > update_limit & strcmp(FILTER,'ON'))
        ...
        if(FILE_FLAG == 1 | FILE_FLAG == 2 | FILE_FLAG == 3)
            qm = eul2quat(att_corrupt(k,:));
        else
            att_corrupt(k,:) = att(k,:) + 0.25*d2r*randn(1,3);
            qm = eul2quat(att_corrupt(k,:));
        end
        
        qerr = quatmult(quatinv(qhat),qm);
        quat_error_vec(k,:) = qerr'; 
        if(qerr(1) < 0)
            qerr = -qerr;
        end
        state_update = L*qerr(2:4);         
       
        qhat = (quatmult(qhat,[1;state_update(1:3)]));
%        if(qhat(1) < 0)
%            qhat = -qhat;
%        end
...
if(0)
    save quat_gain_his.mat t L11 L12 L13 L21 L22 L23 L31 L32 L33 L41 L42 L43 L51 L52 L53 L61 L62 L63;
end

if(1)
    eul_bl = eul;           % Euler Angle Base line.
    if(FILE_FLAG == 1)
        save quat_eulsim_np.mat t L11 L12 L13 L21 L22 L23 L31 L32 L33 L41 L42 L43 L51 L52 L53 L61 L62 L63;
        save quat_eulsim_np.mat Pyaw Ppitch Proll Pp Pq Pr -append;
        save quat_eulsim_np.mat eul_bl yaw the phi p_bias q_bias r_bias -append
    elseif(FILE_FLAG == 2)
        save quat_eulsim_f.mat t L11 L12 L13 L21 L22 L23 L31 L32 L33 L41 L42 L43 L51 L52 L53 L61 L62 L63;
        save quat_eulsim_f.mat Pyaw Ppitch Proll Pp Pq Pr -append; 
        save quat_eulsim_f.mat eul_bl yaw the phi p_bias_r q_bias_r r_bias_r -append
    elseif(FILE_FLAG == 3)
        save quat_eulsim_s.mat t L11 L12 L13 L21 L22 L23 L31 L32 L33 L41 L42 L43 L51 L52 L53 L61 L62 L63;
        save quat_eulsim_s.mat Pyaw Ppitch Proll Pp Pq Pr -append; 
        save quat_eulsim_s.mat eul_bl yaw the phi p_bias_r q_bias_r r_bias_r -append
    end
end
...
figure(5)
h1 = plot(t/60,L41,'r');hold on;grid on;
h2 = plot(t/60,L42,'b');
h3 = plot(t/60,L43,'g');
h_len = legend('L41','L42','L43');
hx = xlabel('Time (min)');hy = ylabel('\delta p Gains');
ht = title('Roll Gyro Bias Channel Gains.');
...
hx = xlabel('Time (min)');hy = ylabel('\delta q Gains');
ht = title('Pitch Gyro Bias Channel Gains.');
...
figure(gcf+1)
subplot(311)
plot(t/60,yaw,'r');grid on;
title(['Euler Angle History']);ylabel('Yaw (deg)')
subplot(312)
plot(t/60,the,'r');ylabel('Pitch (deg)');grid on;
subplot(313)
plot(t/60,phi,'r');ylabel('Roll (deg)');xlabel('Time (min)');
grid on;

if(0) %if(FAST)
    orient tall;orient landscape; print -depsc fast_att_history.eps
else
    orient tall;orient landscape; print -depsc slow_att_history.eps
end
...
if(FAST)
    orient tall;orient landscape;print -depsc fast_quat_gain.eps
else
    orient tall;orient landscape;print -depsc slow_quat_gain.eps
end
