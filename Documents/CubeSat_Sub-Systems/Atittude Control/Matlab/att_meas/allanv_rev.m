	function [tau,av] = allanv_rev(X,fs,cflag,pflag)

%--------------------------------------------------------------------
%
%		[tau,av] = allanv_rev(X,fs,cflag,pflag)
%
%
...
%		cflag = plot 1-sigma confidence limits.  If the eflag
...
%tau = [2:1:0.5*drl/fs]';
%tau = [0.1:1:0.5*drl/fs]';

%t_floor = ceil(1/fs);
%tau = [t_floor:t_floor:0.5*drl/fs]';          % D.G.  Feb 16, 2007
%M = fs*tau;                             %   Number of data points per cluster
%K = fix(drl./M);                        %   Number of clusters

tau_floor = 1/fs;
delta_tau = 1/fs;
tau = [tau_floor:delta_tau:0.5*drl/fs];
M = fix(fs*tau);                             %   Number of data points per cluster
K = fix((drl./M));                        %   Number of clusters
